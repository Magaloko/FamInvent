...
-- ============================================
-- Uses a SECURITY DEFINER helper function to
-- avoid infinite recursion in fm_members policies.
-- ============================================
...
-- ═══════════════════════════════════════════
-- PART 3: HELPER FUNCTION (bypasses RLS)
-- ═══════════════════════════════════════════
-- This function runs with the privileges of the
-- definer (superuser), so it can read fm_members
-- without triggering RLS policies = no recursion.

CREATE OR REPLACE FUNCTION fm_get_my_family_id()
RETURNS UUID
LANGUAGE sql
STABLE
SECURITY DEFINER
...
AS $$
  SELECT family_id FROM fm_members WHERE id = auth.uid() LIMIT 1;
$$;
...
-- ═══════════════════════════════════════════
-- PART 5: RLS POLICIES (using helper function)
-- ═══════════════════════════════════════════
...
CREATE POLICY "Family members can view members"
  ...
  USING (family_id = fm_get_my_family_id());
...
CREATE POLICY "Parents can insert collections"
  ...
  WITH CHECK (family_id = fm_get_my_family_id());
...
CREATE POLICY "Family members can view items"
  ...
  USING (
    collection_id IN (
      SELECT id FROM fm_collections WHERE family_id = fm_get_my_family_id()
    )
  );
...
CREATE POLICY "Family members can view play logs"
  ...
  USING (
    item_id IN (
      ...
      WHERE c.family_id = fm_get_my_family_id()
    )
  );
