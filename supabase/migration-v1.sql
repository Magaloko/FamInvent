...
DO $$ BEGIN
  CREATE POLICY "Family members can view their family"
    ...
    USING (
      id IN (
        SELECT family_id FROM fm_members WHERE id = auth.uid()
      )
    );
EXCEPTION WHEN duplicate_object THEN NULL;
END $$;
...
DO $$ BEGIN
  CREATE POLICY "Family parents can update their family"
    ...
    USING (
      id IN (
        SELECT family_id FROM fm_members WHERE id = auth.uid() AND role = 'parent'
      )
    );
EXCEPTION WHEN duplicate_object THEN NULL;
END $$;
...
DO $$ BEGIN
  CREATE POLICY "Family members can view members"
    ...
    USING (
      family_id IN (
        SELECT family_id FROM fm_members m WHERE m.id = auth.uid()
      )
    );
EXCEPTION WHEN duplicate_object THEN NULL;
END $$;
...
DO $$ BEGIN
  CREATE POLICY "Parents can update members"
    ...
    USING (
      family_id IN (
        SELECT family_id FROM fm_members m WHERE m.id = auth.uid() AND m.role = 'parent'
      )
    );
EXCEPTION WHEN duplicate_object THEN NULL;
END $$;
