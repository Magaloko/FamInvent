...
export default function ItemPage() {
  ...
  async function loadData() {
    ...
    if (d) { setName(d.name); setDescription(d.description || ''); setValue(d.value || ''); setCategory(d.category || '') }
    setLoading(false)
  }

  async function handleSave() {
    const { error } = await updateItem(id, { name: name.trim(), description, value: parseFloat(value) || 0, category })
      ...
      <div className="grid lg:grid-cols-2 gap-6">
        <div className="fm-card-static overflow-hidden">
          <div className="aspect-square bg-fm-bg-input flex items-center justify-center">
            {item.image_url ? <img src={item.image_url} alt={item.name} className="w-full h-full object-cover" /> : <span className="text-8xl">{cat?.icon || '📦'}</span>}
          </div>
        </div>
