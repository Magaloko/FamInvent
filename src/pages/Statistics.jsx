...
import { getStats, getTopToys } from '@/lib/api'
...
export default function Statistics() {
  ...
  useEffect(() => {
    async function load() {
      setLoading(true)
      const [statsRes, toysRes] = await Promise.all([getStats(), getTopToys()])
  ...
  if (loading) return <div className="space-y-4 animate-fade-in"><div className="h-8 w-48 bg-fm-border/30 rounded animate-pulse" /><div className="h-64 bg-fm-border/20 rounded-card animate-pulse" /></div>

  const tooltipStyle = { backgroundColor: 'white', border: '1px solid #F5E6D8', borderRadius: '10px', fontSize: '13px' }
  ...
  return (
    <div className="space-y-6 animate-fade-in">
      ...
      <div className="flex gap-2">
        {tabs.map((t) => (
          <button key={t.key} onClick={() => setTab(t.key)} className={`flex items-center gap-2 px-4 py-2 rounded-full text-sm font-heading font-medium transition-all ${tab === t.key ? 'bg-fm-primary text-white shadow-btn' : 'bg-white text-fm-text-light border border-fm-border'}`}>
        ...
        <div className="space-y-6">
          ...
          <div className="grid grid-cols-2 gap-4">
            <div className="fm-stat-card text-center">
              <p className="text-3xl font-heading font-bold text-fm-primary">{(stats?.totalValue || 0).toFixed(0)} {LABELS.euro}</p>
              <p className="text-sm text-fm-text-muted mt-1">{LABELS.totalValue}</p>
            </div>
            <div className="fm-stat-card text-center">
              <p className="text-3xl font-heading font-bold text-fm-secondary">{stats?.totalItems || 0}</p>
      ...
      {tab === 'play' && (
        <div className="space-y-6">
              ...
              <div className="space-y-3">
                {topToys.map((toy, i) => (
                  <div key={toy.id} className="flex items-center gap-3">
                    <span className="w-8 h-8 rounded-full bg-fm-primary/10 flex items-center justify-center text-sm font-heading font-bold text-fm-primary">{i + 1}</span>
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="mt-1 h-2 bg-fm-bg-input rounded-full overflow-hidden">
                        <div className="h-full rounded-full transition-all" style={{ width: `${(toy.score / (topToys[0]?.score || 1)) * 100}%`, backgroundColor: CHART_COLORS[i % CHART_COLORS.length] }} />
                      </div>
                    </div>
          ...
          <div className="fm-stat-card text-center">
            <p className="text-3xl font-heading font-bold text-fm-yellow">{Math.round((stats?.totalPlayMinutes || 0) / 60)}h</p>
      ...
      {tab === 'categories' && (
        <div className="fm-card-static p-5">
          ...
          {(stats?.categoryDistribution || []).length === 0 ? (
            ...
            <ResponsiveContainer width="100%" height={300}>
              <PieChart>
                <Pie data={stats.categoryDistribution} dataKey="count" nameKey="name" cx="50%" cy="50%" outerRadius={100} innerRadius={40} label={({ name, count }) => `${name} (${count})`} labelLine={false}>
                  {stats.categoryDistribution.map((_, i) => <Cell key={i} fill={CHART_COLORS[i % CHART_COLORS.length]} />)}
                </Pie>
